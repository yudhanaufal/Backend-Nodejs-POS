...
exports.getReturnByToko = async (req, res) => {
  try {
    ...
    const result = await ReturnModel.getByTokoId(parseInt(toko_id), page, limit, filters);
    
    res.json({
      success: true,
      message: `Data return toko ${toko_id} berhasil diambil`,
      data: result.data,
      pagination: result.pagination
    });
  } catch (error) {
    console.error('Get return by toko error:', error);
    res.status(500).json({
      success: false,
      message: "Gagal mengambil data return"
    });
